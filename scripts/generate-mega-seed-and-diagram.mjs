  ...
  return baseAthletes.map((athlete, index) => {
    ...
    const competitions = uniqueBy([
      ...
      ...(index % 5 === 0 ? [{
        ...
        name: primaryGroup === 'peque̱os' ? 'Festival de Fondo' : 'Campeonato Catalan',
...
function buildJogatinaData(athletes, dayStatusRows, seasonKey, now) {
  const playableGroups = ['1500m', '800m', 'peque̱os'];
  const groupCodeBySlug = { '1500m': '15001', '800m': '80001', 'peque̱os': '33001' };
